# 我如何用 AI Agent 完成一款 macOS 产品

![封面：人与 AI Agent 协作开发 macOS 产品](assets/00-cover-ai-agent.png)
...
![Easy Context 在 Finder 中的右键菜单](assets/02-product-finder-menu.png)

*产品要先回到真实场景：它是否让当前目录里的高频操作更顺手。*

把这套协作关系画出来，边界其实很清楚：方向、取舍和最终验收不能外包；探索、执行和证据可以交给 AI；规划、审查与复盘则由双方共同完成。

![人和 AI 的职责边界](assets/03-human-ai-boundary.png)
...
![AI Agent 队伍内部的协作与证据交接](assets/03-agent-collaboration.png)
...
![Easy Context 的设置页面](assets/01-product-settings.png)

*设置页背后并不只是表单：它还承载了多进程共享的命令和配置边界。*

## 第一堵墙：FinderSync 的限制推翻了最初架构
...
![Finder 扩展与宿主 App 的运行时分工](assets/04-runtime-architecture.png)

*Finder 扩展靠近用户上下文，宿主 App 承担复杂动作；两者不必抢着做同一件事。*

## 两次事故：测试通过，不代表真实世界没有问题
...
![从实现、审查到真实验证的反馈闭环](assets/05-feedback-loop.png)

*每一次真实事故，都应该回到测试、文档或任务规则中，成为下一次协作的护栏。*

## 结语：AI 让陌生技术栈变得可进入，但不能替你带队
...
Easy Context 已开源，项目地址：[github.com/yantaolu/easy-context](https://github.com/yantaolu/easy-context)。
