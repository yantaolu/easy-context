...
private final class NameInputPanel: NSObject {
    ...
    init(prefill: String) {
        ...
        let label = NSTextField(labelWithString: "输入文件名：")
        ...
        let cancel = NSButton(title: "取消", target: self, action: #selector(onCancel))
        cancel.frame = NSRect(x: 160, y: 12, width: 85, height: 28)
        cancel.bezelStyle = .rounded
        cancel.keyEquivalent = "\u{1b}" // Esc
        content.addSubview(cancel)

        let create = NSButton(title: "创建", target: self, action: #selector(onCreate))
